...
const StepPbStep10: React.FC<StepPbStep10Props> = ({ 
  savedData, 
  onUpdateData, 
  onNext,
  headerTitle,
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              return (
                ...
                >
                  <Text style={tw`text-base mr-3`}>{countryInfo?.flag || '🏳️'}</Text>
                  <Text style={[tw`text-base flex-1`, { color: colors.c }]}>{item}</Text>
                  <Text style={[tw`text-base`, { color: colors.c_50 }]}>{countryInfo?.code}</Text>
                </TouchableOpacity>
              );
            }}
          />
